...
int p_readlink(const char *link, char *target, size_t target_len)
{
	...
	if (pGetFinalPath == NULL) {
		...
		if (pGetFinalPath == NULL) {
			giterr_set(GITERR_OS,
				QT_TRANSLATE_NOOP("libgit2", "'GetFinalPathNameByHandleW' is not available in this platform"));
			return -1;
		}
	}
	...
	if (hFile == INVALID_HANDLE_VALUE) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Cannot open '%s' for reading"), link);
		return -1;
	}
